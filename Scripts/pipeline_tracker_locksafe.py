...
LOCK_FILE = Path(str(TRACKER_PATH) + ".lock")
...
def _acquire_lock():
    """Acquire a cross-process lock via lockfile creation (Windows-safe)."""
    ...
    while True:
        ...
        if time.time() >= deadline:
            raise TimeoutError(f"Could not acquire tracker lock in {LOCK_TIMEOUT_SECONDS}s")
        time.sleep(0.1)
        ...
        try:
            ...
            try:
                _logging.getLogger(__name__).warning(
                    "pipeline_tracker lock timeout (attempt %d): %s", attempt + 1, _exc
                )
            except Exception:
                pass
            time.sleep(0.4 * (attempt + 1))
